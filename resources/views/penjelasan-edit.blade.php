...
<div class="max-w-2xl mx-auto px-4 py-12 text-left">
    ...
    <form action="{{ route('admin.penjelasan.update') }}" method="POST" class="space-y-6 bg-white p-6 border border-gray-200 rounded-2xl shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="flex flex-col space-y-1.5">
            ...
            <textarea id="konten" name="konten" rows="6" required 
                class="w-full text-sm border-gray-300 rounded-xl focus:ring-pmkBlue focus:border-pmkBlue px-4 py-2.5">{{ old('konten', $penjelasan->konten ?? '') }}</textarea>
        </div>
